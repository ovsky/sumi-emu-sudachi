...
GMainWindow::GMainWindow(std::unique_ptr<QtConfig> config_, bool has_broken_vulkan)
    ...
    const auto sumi_build =
        fmt::format("sumi Development Build | {}-{}", branch_name, description);
...
void GMainWindow::OnGameListDumpRomFS(u64 program_id, const std::string& game_path,
    ...
    const auto dump_dir = target == DumpRomFSTarget::Normal
                              ...
                              : Common::FS::GetSumiPath(Common::FS::SumiPath::SDMCDir) /
                                    "atmosphere" / "contents";
...
bool GMainWindow::MakeShortcutIcoPath(const u64 program_id, const std::string_view game_file_name,
    ...
    out_icon_path /=
        (program_id == 0 ? fmt::format("sumi-{}.{}", game_file_name, ico_extension)
...
void GMainWindow::OnGameListOpenDirectory(const QString& directory) {
    ...
    if (directory == QStringLiteral("SDMC")) {
        fs_path = Common::FS::GetSumiPath(Common::FS::SumiPath::SDMCDir) /
                  "Nintendo/Contents/registered";
    } else if (directory == QStringLiteral("UserNAND")) {
        fs_path = Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) /
...
void GMainWindow::OnOpenSumiFolder() {
    QDesktopServices::openUrl(QUrl::fromLocalFile(QString::fromStdString(
        Common::FS::GetSumiPathString(Common::FS::SumiPath::SumiDir))));
}
...
void GMainWindow::OnInstallFirmware() {
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::information(this, tr("Keys not installed"),
                                 tr("Install decryption keys and restart sumi before attempting "
                                    "to install firmware."));
        return;
    }
