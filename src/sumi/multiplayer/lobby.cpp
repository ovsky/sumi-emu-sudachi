...
#include "sumi/main.h"
#include "sumi/multiplayer/client_room.h"
#include "sumi/multiplayer/lobby.h"
#include "sumi/multiplayer/lobby_p.h"
...
Lobby::Lobby(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() || ui->nickname->text() == QStringLiteral("sumi")) {
        if (!Settings::values.sumi_username.GetValue().empty()) {
            ui->nickname->setText(
                QString::fromStdString(Settings::values.sumi_username.GetValue()));
        } else if (!GetProfileUsername().empty()) {
            ui->nickname->setText(QString::fromStdString(GetProfileUsername()));
        } else {
            ui->nickname->setText(QStringLiteral("sumi"));
        }
    }
...
#ifdef ENABLE_WEB_SERVICE
        if (!Settings::values.sumi_username.GetValue().empty() &&
            !Settings::values.sumi_token.GetValue().empty()) {
