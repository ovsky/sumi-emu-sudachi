...
#include "sumi/multiplayer/direct_connect.h"
...
DirectConnectWindow::DirectConnectWindow(Core::System& system_, QWidget* parent)
    ...
    if (ui->nickname->text().isEmpty() && !Settings::values.sumi_username.GetValue().empty()) {
        // Use sumi Web Service user name as nickname by default
        ui->nickname->setText(QString::fromStdString(Settings::values.sumi_username.GetValue()));
    }
