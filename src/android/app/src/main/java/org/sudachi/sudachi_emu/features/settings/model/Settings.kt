...
object Settings {
    ...
    fun getPlayerString(player: Int): String = SudachiApplication.appContext.getString(R.string.preferences_player, player)
    ...
    enum class EmulationOrientation(
        val int: Int,
    ) {
        Unspecified(0),
        SensorLandscape(5),
        ...
        companion object {
            fun from(int: Int): EmulationOrientation = entries.firstOrNull { it.int == int } ?: Unspecified
        }
    }

    enum class EmulationVerticalAlignment(
        val int: Int,
    ) {
        Top(1),
        Center(0),
        ...
        companion object {
            fun from(int: Int): EmulationVerticalAlignment = entries.firstOrNull { it.int == int } ?: Center
        }
    }
}
