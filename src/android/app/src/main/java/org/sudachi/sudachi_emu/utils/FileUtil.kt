...
object FileUtil {
    ...
    fun getInputStream(path: String) =
    ...
    fun getOutputStream(path: String) =
        if (path.contains("content://")) {
    ...
    @Throws(IOException::class)
    fun getStringFromInputStream(stream: InputStream): String = String(stream.readBytes(), StandardCharsets.UTF_8)

    fun DocumentFile.inputStream(): InputStream = SudachiApplication.appContext.contentResolver.openInputStream(uri)!!

    fun DocumentFile.outputStream(): OutputStream = SudachiApplication.appContext.contentResolver.openOutputStream(uri)!!

    fun Uri.inputStream(): InputStream = SudachiApplication.appContext.contentResolver.openInputStream(this)!!

    fun Uri.outputStream(): OutputStream = SudachiApplication.appContext.contentResolver.openOutputStream(this)!!

    fun Uri.asDocumentFile(): DocumentFile? = DocumentFile.fromSingleUri(SudachiApplication.appContext, this)
}
