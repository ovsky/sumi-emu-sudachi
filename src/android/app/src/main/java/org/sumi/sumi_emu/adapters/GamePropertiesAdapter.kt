...
class GamePropertiesAdapter(
    ...
    override fun getItemViewType(position: Int): Int =
        when (properties[position]) {
            is SubmenuProperty -> PropertyType.Submenu.ordinal
            else -> PropertyType.Installable.ordinal
        }

    inner class SubmenuPropertyViewHolder(
        val binding: CardSimpleOutlinedBinding,
    ...
    inner class InstallablePropertyViewHolder(
        val binding: CardInstallableIconBinding,
    ) : AbstractViewHolder<GameProperty>(binding) {
        override fun bind(model: GameProperty) {
