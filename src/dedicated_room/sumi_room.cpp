...
int main(int argc, char** argv) {
    ...
    if (!announce && enable_sumi_mods) {
        enable_sumi_mods = false;
        LOG_INFO(Network, "Can not enable sumi Moderators for private rooms");
    }
    ...
    if (auto room = network.GetRoom().lock()) {
        ...
        if (!room->Create(room_name, room_description, bind_address, static_cast<u16>(port),
                          ...
                          std::move(verify_backend), ban_list, enable_sumi_mods)) {
