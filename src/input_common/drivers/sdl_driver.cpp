...
public:
    ...
    Common::Input::BatteryLevel GetBatteryLevel(SDL_PowerState power_state, int power_level) {
        switch (power_state) {
        ...
        case SDL_POWERSTATE_ON_BATTERY:
            ...
            else if (power_level >= 10 && power_level < 25)
                return Common::Input::BatteryLevel::Low;
            else if (power_level >= 25 && power_level < 90)
...
SDLDriver::SDLDriver(std::string input_engine_) : InputEngine(std::move(input_engine_)) {
    ...
    if (start_thread && SDL_Init(SDL_INIT_JOYSTICK | SDL_INIT_GAMEPAD) == false) {
    ...
    int count = 0;
    auto devices = SDL_GetJoysticks(&count);
    for (int i = 0; i < count; i++) {
        InitJoystick(devices[i]);
    }
}
